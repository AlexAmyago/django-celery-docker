...
## Original Source
[source](git@github.com:chrisk314/django-celery-docker-example.git)

## Install

```bash
git clone git@github.com:AlexAmyago/django-celery-docker.git
cd django-celery-docker
```
...
the core philosophy of Docker): `app`, `postgres`, `redis`, `celery_beat`, `celery_flower`, and `celery_worker`.
...
check that both `redis:6379` and `app:8000` are reachable before invoking the `celery` command
...
The message broker is specified using the `redis` service hostname which can be resolved by 
any service on the `main` network. Periodic tasks to be scheduled by the `celery_beat` service 
